...
export async function articlesByAuthorRequest(author: string): Promise<DefaultResponseInterface> {
  try {
    const url = `${process.env.REACT_APP_BACKEND_URL}/api/articles?author=${author}`;
    const response = await fetch(url, {
      method: "GET",
      headers: {
        Accept: "application/json",
